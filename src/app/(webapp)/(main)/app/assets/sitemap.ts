...
export default async function assets_sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    return [
        ...
        ...data.images.map((x: any, i: any) => ({
            url: `${process.env.NEXTAUTH_URL}/app/assets/image/${x.name}?id=${x.id}`,
            lastModified: x.updated_at,
            changeFrequency: "weekly",
            priority: 1,
        })),
        ...data.videos.map((x: any, i: any) => ({
            url: `${process.env.NEXTAUTH_URL}/app/assets/video/${x.name}?id=${x.id}`,
            lastModified: x.updated_at,
            changeFrequency: "weekly",
            priority: 1,
        })),
        ...data.audio.map((x: any, i: any) => ({
            url: `${process.env.NEXTAUTH_URL}/app/assets/audio/${x.name}?id=${x.id}`,
            lastModified: x.updated_at,
            changeFrequency: "weekly",
            priority: 1,
        })),
        ...data.text.map((x: any, i: any) => ({
            url: `${process.env.NEXTAUTH_URL}/app/assets/text/${x.name}?id=${x.id}`,
            lastModified: x.updated_at,
            changeFrequency: "weekly",
            priority: 1,
        }))
